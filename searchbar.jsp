                    ...
                    onInitialized: function() {
                        ...
                        $("input[name=價格]:first")
                            ...
                            if($("input[name=價格]:checked").length !== price.length){
                                ...
                                $('.selected-readable').prepend('<span class="item"><i id="tagDelete" class="fa fa-times inputTagprice" value="'+'${price}' + '"></i> ' + selectText +'</span>')
  ...
  $.ajax({ 
    ...
    success: function(res) {
      ...
      for (var i = 0; i < items.length; i++) {
            ...
            // price //價格
            // subject //標題
            // totalSquareFootage //坪
            // notes //機會價格
            var content = '<div class="list-item">' +  
                        '<a href="${pageContext.request.contextPath}/object/' + items[i].objectId+
                        '?s=I21" target="_blank">'+
                        '<div class="list-item-img">' + 
                        '<img src="'+ items[i].coverUrl +'" alt="'+  items[i].short_address+'">' +
                        '<img class="highlight-img" src="'+ items[i].iconImgUrl +'">' +
                        '<span class="list-tag">'+ items[i].objectKind2 +'</span>' +
                        '<span class="list-tag list-price">開價 '+ items[i].price +'萬</span>'  +
                        '</div> </a> <h3>' + items[i].subject + '</h3> <p>總坪數' + items[i].totalSquareFootage +
                        '</p> <p>' + items[i].notes +' +</p> </div>' 

            $(content).appendTo('.list-highlight')
        }
        var button = '<a href="${pageContext.request.contextPath}/home?themeId=31" target="_blank">' +
                     '<div class="highlight-button">看更多機會價</div></a>'
        $(button).appendTo('.list-highlight')
    },
    error: function() {
    },
  }); 
...
function searchEvenFun(record){
   var url ='https://snailhouse.com.tw/service/api/obj/searchrecord?'+record;
