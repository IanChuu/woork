    ...
    SearchableOptionList.prototype = {
        ...
        defaults: {
            ...
            events: {
                ...
                //  function(){            
                // //計算高度
                // alert($(".sol-container").hasClass("sol-active"))
                //  var selectheight = this.$selectionContainer.outerHeight()
                //  // sol-inner-container
                //  if($(".sol-container").hasClass("sol-active")){
                //      $('.displayKeyword').css("top",selectheight-0 );
            ...
            closeOnClick: false, // close when user clicked 'select all' or 'deselect all'
        ...
        _registerWindowEventsIfNeccessary: function() {
            if (!window[this.WINDOW_EVENTS_KEY]) {
                $(document).click(function(event) {
                    ...
                    $('.sol-active')
                        .not($clickedWithinThisSolContainer, $('#deleteItem'))
        ...
        _initializeUiElements: function() {
            ...
            this.$input = $('<input id="selectDisplay" type="text"/>')
            ...
            if (this.config.resultsContainer) {
                ...
                if (this.config.showSelectionBelowList) {
                    // this.$showSelectionContainer.insertAfter(this.$selection)
                    this.$showSelectionContainer.insertAfter($('#print-item'))
        ...
        _renderOptiongroup: function(solOptiongroup) {
            var self = this,
                ...
                $group = $('<div class="sol-optiongroup"/>').append('<form>'),
                $form = $('<form>').append($groupCaption,
                    $closetag);
                //('<div class="sol-optiongroup"/>').wrap('<form>');
                $groupItem = $group.append($form);
        ...
        _addSelectionDisplayItem: function($changedItem) {
            ...
            if (!$existingDisplayItem) {
                ...
                $('<span id="deleteItem" class="sol-quick-delete"/>')
